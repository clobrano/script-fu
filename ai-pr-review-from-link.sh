...
notify-send --app-name "$APP_NAME" "Request accepted. Getting PR data..."
...
# drop number (https://github.com/openshift/cluster-etcd-operator/pull)
TRIM_LINK=$(dirname "$PR_LINK")
# drop "pull" (https://github.com/openshift/cluster-etcd-operator)
TRIM_LINK=$(dirname "$TRIM_LINK")
PR_PROJ=$(basename "$TRIM_LINK")
# drop project name (https://github.com/openshift)
TRIM_LINK=$(dirname "$TRIM_LINK")
...
notify-send --app-name "$APP_NAME" "$PR_ORG/$PR_PROJ PR $PR_NUM. Cloned in $TMP_DIR. Starting review"

claude --print "/review $PR_NUM" > "$PR_REV_FILE"

ACTION_CHOICE=$(notify-send  "$APP_NAME" "Review for $PR_ORG/$PR_PROJ PR $PR_NUM ready Open the file?" \
    --expire-time=30000 \
    --action="yes=Yes" \
    --action="no=No" \
    --wait)
